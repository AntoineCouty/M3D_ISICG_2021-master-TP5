...
namespace M3D_ISICG
{
	...
	Material TriangleMeshModel::_loadMaterial( const aiMaterial * const p_mtl )
	{
		...
		// ===================================================== NORMAL
		if ( p_mtl->GetTextureCount( aiTextureType_NORMALS ) > 0 ) // Normal ?
		{
		...
		{
			material._hasOpacityMap = false;
		}

		std::partition( _meshes.begin(),
						_meshes.end(),
						[]( const TriangleMesh & p_mesh ) -> bool { return p_mesh._material._hasOpacityMap;} );
	...
	Texture TriangleMeshModel::_loadTexture( const aiString & p_path, const std::string & p_type )
	{
		...
		if ( image.load( fullPath ) )
		{
			...
			int mipmapLevel = glm::max(glm::log2((float) glm::max( image._width, image._height ) ), 1.f);
